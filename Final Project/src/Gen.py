import random
import uuid
from datetime import datetime, timedelta
import os
import re
from faker import Faker

class BookmateDataGenerator:
    def __init__(self):
        self.data_file = "data.sql"
        
        # Configurar Faker con locales en español
        self.fake = Faker(['es_ES', 'es_MX', 'es_AR'])
        
        # Cache para libros existentes
        self.libros_existentes = []
        self.libros_cargados = False
        
        # Listas específicas que mantenemos para el contexto literario
        self.generos_literarios = [
            'Ficción', 'No ficción', 'Misterio', 'Romance', 'Ciencia ficción', 
            'Fantasía', 'Biografía', 'Historia', 'Terror', 'Aventura',
            'Drama', 'Comedia', 'Ensayo', 'Poesía', 'Thriller'
        ]
        
        self.editoriales_famosas = [
            "Editorial Planeta", "Penguin Random House", "Alfaguara", "Anagrama",
            "Seix Barral", "Tusquets", "Cátedra", "Espasa", "Crítica", "Destino",
            "Salamandra", "Minotauro", "Acantilado", "Berenice", "Impedimenta"
        ]
        
        self.idiomas = ["Español", "Inglés", "Francés", "Portugués", "Italiano", "Alemán", "Catalán"]
        
        # Plantillas para generar contenido más realista
        self.tipos_biografia = [
            "Escritor y periodista reconocido por su estilo narrativo único",
            "Novelista contemporáneo con múltiples premios internacionales",
            "Autor de obras que han marcado la literatura hispanoamericana",
            "Poeta y ensayista cuya obra trasciende generaciones",
            "Dramaturgo y novelista de gran influencia en la literatura mundial",
            "Escritor cuya obra explora temas sociales y políticos profundos",
            "Autor de cuentos y novelas que han sido traducidos a múltiples idiomas"
        ]
        
        self.tipos_sinopsis = [
            "Una historia fascinante que explora los límites del tiempo y el espacio",
            "Un relato conmovedor sobre el amor y la pérdida en tiempos difíciles",
            "Una obra que combina realismo mágico con crítica social profunda",
            "Un viaje literario a través de la memoria y la identidad",
            "Una novela que desafía las convenciones narrativas tradicionales",
            "Un texto que reflexiona sobre la condición humana en el mundo moderno",
            "Una historia épica que abarca generaciones de una familia extraordinaria"
        ]
        
        self.tipos_notificaciones = [
            "Nuevo libro disponible en tu género favorito",
            "Recordatorio: continúa leyendo tu libro actual",
            "Oferta especial disponible por tiempo limitado",
            "Te recomendamos este libro basado en tus lecturas",
            "Actualización disponible para la aplicación",
            "Tienes un nuevo seguidor en tu perfil",
            "Alguien comentó en tu reseña reciente",
            "Tu lista de lectura ha sido actualizada"
        ]

    def generar_uuid(self):
        """Genera un UUID aleatorio"""
        return str(uuid.uuid4())
    
    def leer_libros_existentes(self):
        """Lee el archivo data.sql y extrae los libros existentes"""
        if not os.path.exists(self.data_file):
            print(f"Archivo {self.data_file} no encontrado. No hay libros existentes.")
            return []
        
        libros = []
        try:
            with open(self.data_file, 'r', encoding='utf-8') as f:
                contenido = f.read()
            
            # Buscar todas las líneas que contienen INSERT INTO "book"
            # Patrón más flexible para capturar los INSERT de libros
            lineas = contenido.split('\n')
            
            for i, linea in enumerate(lineas):
                if 'INSERT INTO "book"' in linea:
                    # Buscar la línea VALUES correspondiente
                    valores_linea = ""
                    j = i
                    while j < len(lineas) and 'VALUES' not in lineas[j]:
                        j += 1
                    
                    if j < len(lineas):
                        valores_linea = lineas[j]
                        
                        # Extraer valores usando regex más simple
                        patron = r"VALUES\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,.*?,.*?,\s*(true|false)\s*,"
                        match = re.search(patron, valores_linea, re.IGNORECASE)
                        
                        if match:
                            book_id = match.group(1)
                            title = match.group(2)
                            author_id = match.group(3)
                            is_audiobook = match.group(4).lower() == 'true'
                            
                            libros.append({
                                'book_id': book_id,
                                'title': title,
                                'author_id': author_id,
                                'is_audiobook': is_audiobook
                            })
            
            print(f"📚 Se encontraron {len(libros)} libros existentes en el archivo.")
            if libros:
                print("   Libros disponibles:")
                for libro in libros[:5]:  # Mostrar solo los primeros 5
                    print(f"   - {libro['title']}")
                if len(libros) > 5:
                    print(f"   ... y {len(libros) - 5} más")
            
        except Exception as e:
            print(f"❌ Error al leer libros existentes: {e}")
            return []
        
        return libros
    
    def cargar_libros_existentes(self):
        """Carga los libros existentes si no se han cargado ya"""
        if not self.libros_cargados:
            self.libros_existentes = self.leer_libros_existentes()
            self.libros_cargados = True
        return self.libros_existentes
    
    def obtener_libros_aleatorios(self, cantidad_min=0, cantidad_max=5):
        """Obtiene una selección aleatoria de libros existentes"""
        libros = self.cargar_libros_existentes()
        
        if not libros:
            return []
        
        # Determinar cuántos libros seleccionar
        cantidad = random.randint(cantidad_min, min(cantidad_max, len(libros)))
        
        if cantidad == 0:
            return []
        
        # Seleccionar libros aleatorios
        return random.sample(libros, cantidad)

    def generar_fecha_aleatoria(self, dias_atras=365):
        """Genera una fecha aleatoria dentro de los últimos días especificados"""
        fecha_base = datetime.now()
        fecha_aleatoria = fecha_base - timedelta(days=random.randint(0, dias_atras))
        return fecha_aleatoria.strftime('%Y-%m-%d %H:%M:%S')

    def generar_isbn(self):
        """Genera un ISBN-13 aleatorio"""
        return f"978-{random.randint(10,99)}-{random.randint(100,999)}-{random.randint(1000,9999)}-{random.randint(0,9)}"

    def generar_usuario(self):
        """Genera un usuario aleatorio usando Faker"""
        user_id = self.generar_uuid()
        nombre = self.fake.name()
        password = self.fake.password(length=12)
        is_premium = self.fake.boolean()
        user_type = random.randint(1, 3)
        
        sql = f"""
-- Usuario generado aleatoriamente
INSERT INTO "user" ("user_id", "password", "name", "is_premium", "user_type") 
VALUES ('{user_id}', '{password}', '{nombre}', {is_premium}, {user_type});
"""
        return sql, user_id

    def generar_autor(self):
        """Genera un autor aleatorio usando Faker"""
        author_id = self.generar_uuid()
        nombre = self.fake.name()
        # Generar biografía más realista
        biografia = f"{random.choice(self.tipos_biografia)}. {self.fake.text(max_nb_chars=200)}"
        nacionalidad = random.randint(1, 200)  # Asumiendo que hay países del 1 al 200
        
        sql = f"""
-- Autor generado aleatoriamente
INSERT INTO "author" ("author_id", "name", "biography", "nationality") 
VALUES ('{author_id}', '{nombre}', '{biografia}', {nacionalidad});
"""
        return sql, author_id

    def generar_editorial(self):
        """Genera una editorial aleatoria usando Faker"""
        publisher_id = self.generar_uuid()
        nombre = random.choice(self.editoriales_famosas)
        country_id = random.randint(1, 200)
        founded_year = self.fake.year()
        website = self.fake.url()
        descripcion = f"Editorial {nombre} fundada en {founded_year}. {self.fake.company_suffix()}"
        
        sql = f"""
-- Editorial generada aleatoriamente
INSERT INTO "publisher" ("publisher_id", "name", "country_id", "founded_year", "website", "description") 
VALUES ('{publisher_id}', '{nombre}', {country_id}, {founded_year}, '{website}', '{descripcion}');
"""
        return sql, publisher_id

    def generar_libro(self, author_id, user_id, publisher_id):
        """Genera un libro aleatorio usando Faker"""
        book_id = self.generar_uuid()
        # Generar título más realista
        titulo = self.fake.catch_phrase()
        sinopsis = f"{random.choice(self.tipos_sinopsis)}. {self.fake.text(max_nb_chars=300)}"
        idioma = random.choice(self.idiomas)
        is_audiobook = self.fake.boolean()
        file_url = self.fake.url()
        published_year = f"{self.fake.year()}-01-01"
        isbn = self.generar_isbn()
        
        sql = f"""
-- Libro generado aleatoriamente
INSERT INTO "book" ("book_id", "title", "author", "synopsis", "language", "is_audiobook", "file_url", "uploaded_by", "publisher_id", "published_year", "isbn") 
VALUES ('{book_id}', '{titulo}', '{author_id}', '{sinopsis}', '{idioma}', {is_audiobook}, '{file_url}', '{user_id}', '{publisher_id}', '{published_year}', '{isbn}');
"""
        return sql, book_id, is_audiobook

    def generar_audiobook(self, book_id):
        """Genera datos de audiobook usando Faker"""
        duration = random.randint(300, 1200)  # 5 a 20 horas en minutos
        narrator = self.fake.name()
        audio_file_url = self.fake.url()
        
        sql = f"""
-- Audiobook generado aleatoriamente
INSERT INTO "audiobook" ("book_id", "duration", "narrator", "audio_file_url") 
VALUES ('{book_id}', {duration}, '{narrator}', '{audio_file_url}');
"""
        return sql

    def generar_metricas_libro(self, book_id):
        """Genera métricas para un libro usando Faker"""
        total_reads = self.fake.random_int(min=50, max=2000)
        total_listens = self.fake.random_int(min=0, max=1000)
        total_favorites = self.fake.random_int(min=10, max=500)
        total_reviews = self.fake.random_int(min=5, max=300)
        average_rating = round(random.uniform(3.0, 5.0), 1)
        last_read_at = self.fake.date_time_between(start_date='-30d', end_date='now').strftime('%Y-%m-%d %H:%M:%S')
        
        sql = f"""
-- Métricas de libro generadas aleatoriamente
INSERT INTO "book_metrics" ("book_id", "total_reads", "total_listens", "total_favorites", "total_reviews", "average_rating", "last_read_at") 
VALUES ('{book_id}', {total_reads}, {total_listens}, {total_favorites}, {total_reviews}, {average_rating}, '{last_read_at}');
"""
        return sql

    def generar_review(self, user_id, book_id):
        """Genera una reseña aleatoria usando Faker"""
        review_id = self.generar_uuid()
        rating = self.fake.random_int(min=1, max=5)
        comentario = self.fake.sentence(nb_words=8)
        
        sql = f"""
-- Reseña generada aleatoriamente
INSERT INTO "review" ("review_id", "rating", "comment", "user_id", "book_id") 
VALUES ('{review_id}', {rating}, '{comentario}', '{user_id}', '{book_id}');
"""
        return sql

    def generar_favorito(self, user_id, book_id):
        """Genera un favorito aleatorio"""
        favorite_id = self.generar_uuid()
        
        sql = f"""
-- Favorito generado aleatoriamente
INSERT INTO "favorite" ("favorite_id", "user_id", "book_id") 
VALUES ('{favorite_id}', '{user_id}', '{book_id}');
"""
        return sql

    def generar_progreso_lectura(self, user_id, book_id):
        """Genera progreso de lectura usando Faker"""
        pages_read = self.fake.random_int(min=0, max=500)
        minutes_listened = self.fake.random_int(min=0, max=300)
        last_accessed = self.fake.date_time_between(start_date='-7d', end_date='now').strftime('%Y-%m-%d %H:%M:%S')
        
        sql = f"""
-- Progreso de lectura generado aleatoriamente
INSERT INTO "user_book" ("user_id", "book_id", "pages_read", "minutes_listened", "last_accessed") 
VALUES ('{user_id}', '{book_id}', {pages_read}, {minutes_listened}, '{last_accessed}');
"""
        return sql

    def generar_lista_lectura(self, user_id):
        """Genera una lista de lectura usando Faker"""
        list_id = self.generar_uuid()
        nombre = f"{self.fake.word().title()} {random.choice(self.generos_literarios)}"
        descripcion = self.fake.sentence(nb_words=10)
        is_public = self.fake.boolean()
        
        sql = f"""
-- Lista de lectura generada aleatoriamente
INSERT INTO "reading_list" ("list_id", "name", "description", "created_by", "is_public") 
VALUES ('{list_id}', '{nombre}', '{descripcion}', '{user_id}', {is_public});
"""
        return sql, list_id

    def generar_item_lista(self, list_id, book_id):
        """Genera un item para una lista de lectura"""
        sql = f"""
-- Item de lista generado aleatoriamente
INSERT INTO "reading_list_item" ("list_id", "book_id") 
VALUES ('{list_id}', '{book_id}');
"""
        return sql

    def generar_notificacion(self, user_id):
        """Genera una notificación usando Faker"""
        notification_id = self.generar_uuid()
        mensaje = random.choice(self.tipos_notificaciones)
        is_read = self.fake.boolean()
        notification_type = random.randint(1, 4)
        
        sql = f"""
-- Notificación generada aleatoriamente
INSERT INTO "notification" ("notification_id", "message", "is_read", "user_id", "notification_type") 
VALUES ('{notification_id}', '{mensaje}', {is_read}, '{user_id}', {notification_type});
"""
        return sql

    def generar_metricas_usuario(self, user_id, libros_leidos, reseñas_escritas, favoritos_count):
        """Genera métricas del usuario basadas en su actividad"""
        # Calcular métricas basadas en la actividad real del usuario
        books_read = libros_leidos
        minutes_listened = random.randint(0, libros_leidos * 200) if libros_leidos > 0 else 0
        reviews_written = reseñas_escritas
        favorites_count_real = favoritos_count
        total_pages_read = random.randint(0, libros_leidos * 300) if libros_leidos > 0 else 0
        active_days = random.randint(1, min(365, libros_leidos * 30)) if libros_leidos > 0 else random.randint(1, 7)
        last_active_at = self.fake.date_time_between(start_date='-7d', end_date='now').strftime('%Y-%m-%d %H:%M:%S')
        
        sql = f"""
-- Métricas de usuario generadas aleatoriamente
INSERT INTO "user_metrics" ("user_id", "books_read", "minutes_listened", "reviews_written", "favorites_count", "total_pages_read", "active_days", "last_active_at") 
VALUES ('{user_id}', {books_read}, {minutes_listened}, {reviews_written}, {favorites_count_real}, {total_pages_read}, {active_days}, '{last_active_at}');
"""
        return sql

    def determinar_perfil_usuario(self):
        """Determina el perfil de actividad del usuario de forma aleatoria"""
        perfiles = {
            'nuevo': {
                'probabilidad': 0.25,
                'libros': (0, 1),
                'reseñas': (0, 0),
                'favoritos': (0, 0),
                'listas': (0, 1),
                'notificaciones': (0, 2),
                'descripcion': 'Usuario nuevo con poca o ninguna actividad'
            },
            'casual': {
                'probabilidad': 0.40,
                'libros': (1, 3),
                'reseñas': (0, 2),
                'favoritos': (0, 2),
                'listas': (1, 2),
                'notificaciones': (1, 4),
                'descripcion': 'Usuario casual con actividad moderada'
            },
            'activo': {
                'probabilidad': 0.25,
                'libros': (3, 6),
                'reseñas': (2, 5),
                'favoritos': (2, 4),
                'listas': (2, 4),
                'notificaciones': (3, 7),
                'descripcion': 'Usuario activo con buena participación'
            },
            'power_user': {
                'probabilidad': 0.10,
                'libros': (5, 10),
                'reseñas': (4, 8),
                'favoritos': (3, 6),
                'listas': (3, 6),
                'notificaciones': (5, 10),
                'descripcion': 'Usuario muy activo, gran lector'
            }
        }
        
        # Seleccionar perfil basado en probabilidades
        rand = random.random()
        acumulado = 0
        for perfil, config in perfiles.items():
            acumulado += config['probabilidad']
            if rand <= acumulado:
                return perfil, config
        
        # Fallback al perfil casual
        return 'casual', perfiles['casual']

    def generar_datos_completos(self):
        """Genera un conjunto completo de datos relacionados con perfiles realistas"""
        datos_sql = []
        
        # Determinar perfil del usuario
        perfil, config = self.determinar_perfil_usuario()
        
        # Generar usuario
        sql_usuario, user_id = self.generar_usuario()
        datos_sql.append(sql_usuario)
        
        # Cargar libros existentes
        libros_existentes = self.cargar_libros_existentes()
        
        # Decidir si usar libros existentes o generar nuevos
        usar_libros_existentes = len(libros_existentes) > 0 and random.random() < 0.7  # 70% probabilidad de usar existentes
        
        if usar_libros_existentes:
            # Usar libros existentes
            min_libros = max(1, config['libros'][0])
            max_libros = max(1, min(config['libros'][1], len(libros_existentes)))
            
            # Asegurar que el rango sea válido
            if min_libros > max_libros:
                min_libros = max_libros
            
            num_libros_interaccion = random.randint(min_libros, max_libros)
            libros_para_interaccion = random.sample(libros_existentes, num_libros_interaccion) if num_libros_interaccion > 0 else []
            
            # Convertir a formato esperado
            libros_generados = [(libro['book_id'], libro['is_audiobook']) for libro in libros_para_interaccion]
            
            print(f"Usuario {user_id[:8]}... usando {len(libros_generados)} libros existentes")
            
        else:
            # Generar nuevos libros (código original)
            autores = []
            editoriales = []
            
            # Generar al menos un autor y editorial
            sql_autor, author_id = self.generar_autor()
            datos_sql.append(sql_autor)
            autores.append(author_id)
            
            sql_editorial, publisher_id = self.generar_editorial()
            datos_sql.append(sql_editorial)
            editoriales.append(publisher_id)
            
            # Generar libros según el perfil del usuario
            num_libros = random.randint(config['libros'][0], config['libros'][1])
            libros_generados = []
            
            for i in range(num_libros):
                # Ocasionalmente crear nuevos autores/editoriales para variedad
                if random.random() < 0.3 and i > 0:
                    sql_autor, author_id = self.generar_autor()
                    datos_sql.append(sql_autor)
                    autores.append(author_id)
                
                if random.random() < 0.2 and i > 0:
                    sql_editorial, publisher_id = self.generar_editorial()
                    datos_sql.append(sql_editorial)
                    editoriales.append(publisher_id)
                
                # Seleccionar autor y editorial aleatoriamente de los disponibles
                author_id = random.choice(autores)
                publisher_id = random.choice(editoriales)
                
                # Generar libro
                sql_libro, book_id, is_audiobook = self.generar_libro(author_id, user_id, publisher_id)
                datos_sql.append(sql_libro)
                libros_generados.append((book_id, is_audiobook))
                
                # Si es audiobook, generar datos de audiobook
                if is_audiobook:
                    sql_audiobook = self.generar_audiobook(book_id)
                    datos_sql.append(sql_audiobook)
                
                # Generar métricas del libro
                sql_metricas = self.generar_metricas_libro(book_id)
                datos_sql.append(sql_metricas)
                
                # Generar géneros para el libro (1-3 géneros aleatorios)
                num_generos = random.randint(1, 3)
                generos_usados = []
                for _ in range(num_generos):
                    genero = random.randint(1, 8)
                    if genero not in generos_usados:
                        generos_usados.append(genero)
                        sql_genero = f"""
-- Género de libro generado aleatoriamente
INSERT INTO "genre_book" ("book_id", "genre_type_id") 
VALUES ('{book_id}', {genero});
"""
                        datos_sql.append(sql_genero)
                
                # Generar tags para el libro (1-2 tags aleatorios)
                num_tags = random.randint(1, 2)
                tags_usados = []
                for _ in range(num_tags):
                    tag = random.randint(1, 6)
                    if tag not in tags_usados:
                        tags_usados.append(tag)
                        sql_tag = f"""
-- Tag de libro generado aleatoriamente
INSERT INTO "book_tag" ("book_id", "tag_id") 
VALUES ('{book_id}', {tag});
"""
                        datos_sql.append(sql_tag)
            
            print(f"Usuario {user_id[:8]}... generando {len(libros_generados)} libros nuevos")
        
        # Resto del código para generar interacciones...
        # ...existing code...
        # Generar reseñas según el perfil
        num_reseñas = random.randint(config['reseñas'][0], config['reseñas'][1])
        if num_reseñas > 0 and libros_generados:
            # Puede reseñar libros que no necesariamente generó (más realista)
            libros_para_reseñar = random.sample(libros_generados, min(num_reseñas, len(libros_generados)))
            for book_id, _ in libros_para_reseñar:
                sql_review = self.generar_review(user_id, book_id)
                datos_sql.append(sql_review)
        
        # Generar favoritos según el perfil
        num_favoritos = random.randint(config['favoritos'][0], config['favoritos'][1])
        if num_favoritos > 0 and libros_generados:
            libros_para_favoritos = random.sample(libros_generados, min(num_favoritos, len(libros_generados)))
            for book_id, _ in libros_para_favoritos:
                sql_favorito = self.generar_favorito(user_id, book_id)
                datos_sql.append(sql_favorito)
        
        # Generar progreso de lectura para libros que el usuario ha interactuado
        libros_con_progreso = random.sample(libros_generados, min(len(libros_generados), max(1, len(libros_generados) // 2)))
        for book_id, _ in libros_con_progreso:
            sql_progreso = self.generar_progreso_lectura(user_id, book_id)
            datos_sql.append(sql_progreso)
        
        # Generar listas de lectura según el perfil
        num_listas = random.randint(config['listas'][0], config['listas'][1])
        for _ in range(num_listas):
            sql_lista, list_id = self.generar_lista_lectura(user_id)
            datos_sql.append(sql_lista)
            
            # Agregar algunos libros a la lista (no todos)
            if libros_generados:
                num_libros_en_lista = random.randint(1, min(3, len(libros_generados)))
                libros_para_lista = random.sample(libros_generados, num_libros_en_lista)
                for book_id, _ in libros_para_lista:
                    sql_item_lista = self.generar_item_lista(list_id, book_id)
                    datos_sql.append(sql_item_lista)
        
        # Generar notificaciones según el perfil
        num_notificaciones = random.randint(config['notificaciones'][0], config['notificaciones'][1])
        for _ in range(num_notificaciones):
            sql_notificacion = self.generar_notificacion(user_id)
            datos_sql.append(sql_notificacion)
        
        # Generar métricas del usuario basadas en su actividad
        if libros_generados:
            sql_metricas_usuario = self.generar_metricas_usuario(user_id, len(libros_generados), num_reseñas, num_favoritos)
            datos_sql.append(sql_metricas_usuario)
        
        return datos_sql

    def generar_usuario_con_libros_existentes(self):
        """Genera un usuario que interactúa solo con libros existentes"""
        datos_sql = []
        
        # Cargar libros existentes
        libros_existentes = self.cargar_libros_existentes()
        
        if not libros_existentes:
            print("❌ No hay libros existentes en el archivo data.sql")
            print("   Primero ejecuta el generador normal para crear algunos libros")
            return []
        
        # Determinar perfil del usuario
        perfil, config = self.determinar_perfil_usuario()
        
        # Generar usuario
        sql_usuario, user_id = self.generar_usuario()
        datos_sql.append(sql_usuario)
        
        # Seleccionar libros existentes para interactuar
        min_libros = max(1, config['libros'][0])
        max_libros = max(1, min(config['libros'][1], len(libros_existentes)))
        
        # Asegurar que el rango sea válido
        if min_libros > max_libros:
            min_libros = max_libros
        
        num_libros_interaccion = random.randint(min_libros, max_libros)
        
        libros_para_interaccion = random.sample(libros_existentes, num_libros_interaccion)
        libros_generados = [(libro['book_id'], libro['is_audiobook']) for libro in libros_para_interaccion]
        
        print(f"👤 Usuario {user_id[:8]}... interactuando con {len(libros_generados)} libros existentes")
        for libro in libros_para_interaccion:
            print(f"   📚 {libro['title']}")
        
        # Generar reseñas según el perfil
        num_reseñas = random.randint(config['reseñas'][0], config['reseñas'][1])
        if num_reseñas > 0 and libros_generados:
            libros_para_reseñar = random.sample(libros_generados, min(num_reseñas, len(libros_generados)))
            for book_id, _ in libros_para_reseñar:
                sql_review = self.generar_review(user_id, book_id)
                datos_sql.append(sql_review)
        
        # Generar favoritos según el perfil
        num_favoritos = random.randint(config['favoritos'][0], config['favoritos'][1])
        if num_favoritos > 0 and libros_generados:
            libros_para_favoritos = random.sample(libros_generados, min(num_favoritos, len(libros_generados)))
            for book_id, _ in libros_para_favoritos:
                sql_favorito = self.generar_favorito(user_id, book_id)
                datos_sql.append(sql_favorito)
        
        # Generar progreso de lectura para algunos libros
        libros_con_progreso = random.sample(libros_generados, min(len(libros_generados), max(1, len(libros_generados) // 2)))
        for book_id, _ in libros_con_progreso:
            sql_progreso = self.generar_progreso_lectura(user_id, book_id)
            datos_sql.append(sql_progreso)
        
        # Generar listas de lectura según el perfil
        num_listas = random.randint(config['listas'][0], config['listas'][1])
        for _ in range(num_listas):
            sql_lista, list_id = self.generar_lista_lectura(user_id)
            datos_sql.append(sql_lista)
            
            # Agregar algunos libros a la lista
            if libros_generados:
                num_libros_en_lista = random.randint(1, min(3, len(libros_generados)))
                libros_para_lista = random.sample(libros_generados, num_libros_en_lista)
                for book_id, _ in libros_para_lista:
                    sql_item_lista = self.generar_item_lista(list_id, book_id)
                    datos_sql.append(sql_item_lista)
        
        # Generar notificaciones según el perfil
        num_notificaciones = random.randint(config['notificaciones'][0], config['notificaciones'][1])
        for _ in range(num_notificaciones):
            sql_notificacion = self.generar_notificacion(user_id)
            datos_sql.append(sql_notificacion)
        
        # Generar métricas del usuario basadas en su actividad
        sql_metricas_usuario = self.generar_metricas_usuario(user_id, len(libros_generados), num_reseñas, num_favoritos)
        datos_sql.append(sql_metricas_usuario)
        
        return datos_sql, perfil, config

    def guardar_datos(self, datos_sql, perfil_info=None):
        """Guarda los datos generados en el archivo data.sql"""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        with open(self.data_file, 'a', encoding='utf-8') as f:
            f.write(f"\n\n-- Datos generados aleatoriamente el {timestamp}\n")
            if perfil_info:
                f.write(f"-- Perfil de usuario: {perfil_info}\n")
            for sql in datos_sql:
                f.write(sql)
        
        print(f"📁 Datos generados y guardados en {self.data_file}")
        if perfil_info:
            print(f"👤 Perfil de usuario: {perfil_info}")

    def generar_multiples_usuarios_existentes(self, cantidad=1):
        """Genera múltiples usuarios que interactúan solo con libros existentes"""
        print(f"🔄 Generando {cantidad} usuario(s) que interactúan con libros existentes...")
        
        # Verificar que hay libros existentes
        libros_existentes = self.cargar_libros_existentes()
        if not libros_existentes:
            print("❌ No hay libros existentes en el archivo data.sql")
            print("   Primero ejecuta el generador normal para crear algunos libros")
            return
        
        print(f"📚 Libros disponibles: {len(libros_existentes)}")
        
        perfiles_generados = {'nuevo': 0, 'casual': 0, 'activo': 0, 'power_user': 0}
        usuarios_generados = 0
        
        for i in range(cantidad):
            print(f"\n🔄 Generando usuario {i+1}/{cantidad}...")
            
            try:
                # Generar usuario con libros existentes
                result = self.generar_usuario_con_libros_existentes()
                
                if isinstance(result, tuple) and len(result) == 3:
                    datos_sql, perfil, config = result
                    
                    if datos_sql:
                        perfiles_generados[perfil] += 1
                        usuarios_generados += 1
                        
                        # Guardar con información del perfil
                        perfil_info = f"{perfil.replace('_', ' ').title()} - {config['descripcion']} (Solo libros existentes)"
                        self.guardar_datos(datos_sql, perfil_info)
                        print(f"✅ Usuario {i+1} generado exitosamente")
                    else:
                        print(f"❌ Error al generar el usuario {i+1}")
                else:
                    print(f"❌ Error en la estructura de respuesta para usuario {i+1}")
                    
            except Exception as e:
                print(f"❌ Error al generar usuario {i+1}: {e}")
                continue
            
        print(f"\n✅ ¡Generación completada! Se generaron {usuarios_generados} usuarios con libros existentes.")
        
        if usuarios_generados > 0:
            print("\n📊 Distribución de perfiles generados:")
            for perfil, count in perfiles_generados.items():
                if count > 0:
                    percentage = (count / usuarios_generados) * 100
                    print(f"  {perfil.replace('_', ' ').title()}: {count} ({percentage:.1f}%)")

    def ejecutar(self):
        """Ejecuta el generador de datos"""
        # Verificar e instalar Faker si es necesario
        try:
            from faker import Faker
        except ImportError:
            print("Faker no está instalado. Instalando...")
            import subprocess
            import sys
            subprocess.check_call([sys.executable, "-m", "pip", "install", "faker"])
            print("Faker instalado exitosamente.")
            from faker import Faker
        
        print("Generando datos aleatorios para la base de datos Bookmate...")
        
        # Verificar si el archivo existe
        if not os.path.exists(self.data_file):
            print(f"Archivo {self.data_file} no encontrado. Creando nuevo archivo...")
            with open(self.data_file, 'w', encoding='utf-8') as f:
                f.write("-- Archivo de datos para Bookmate\n")
        
        # Generar datos con perfil
        perfil, config = self.determinar_perfil_usuario()
        datos_sql = self.generar_datos_completos()
        
        # Guardar datos con información del perfil
        perfil_info = f"{perfil.replace('_', ' ').title()} - {config['descripcion']}"
        self.guardar_datos(datos_sql, perfil_info)
        
        print("¡Generación completada exitosamente!")
        print(f"Se generaron {len(datos_sql)} consultas SQL nuevas.")

    def generar_multiples_registros(self, cantidad=1):
        """Genera múltiples conjuntos de datos aleatorios"""
        print(f"Generando {cantidad} conjunto(s) de datos aleatorios...")
        
        perfiles_generados = {'nuevo': 0, 'casual': 0, 'activo': 0, 'power_user': 0}
        
        for i in range(cantidad):
            print(f"Generando conjunto {i+1}/{cantidad}...")
            
            # Determinar perfil para este usuario
            perfil, config = self.determinar_perfil_usuario()
            perfiles_generados[perfil] += 1
            
            # Generar datos
            datos_sql = self.generar_datos_completos()
            
            # Guardar con información del perfil
            perfil_info = f"{perfil.replace('_', ' ').title()} - {config['descripcion']}"
            self.guardar_datos(datos_sql, perfil_info)
            
        print(f"¡Generación completada! Se generaron {cantidad} conjuntos de datos.")
        print("\n📊 Distribución de perfiles generados:")
        for perfil, count in perfiles_generados.items():
            if count > 0:
                percentage = (count / cantidad) * 100
                print(f"  {perfil.replace('_', ' ').title()}: {count} ({percentage:.1f}%)")

    def mostrar_estadisticas(self):
        """Muestra estadísticas del archivo de datos"""
        if not os.path.exists(self.data_file):
            print("El archivo de datos no existe.")
            return
            
        with open(self.data_file, 'r', encoding='utf-8') as f:
            contenido = f.read()
            
        # Contar diferentes tipos de INSERT
        tipos_inserts = {
            'usuarios': contenido.count('INSERT INTO "user"'),
            'autores': contenido.count('INSERT INTO "author"'),
            'editoriales': contenido.count('INSERT INTO "publisher"'),
            'libros': contenido.count('INSERT INTO "book"'),
            'audiobooks': contenido.count('INSERT INTO "audiobook"'),
            'reseñas': contenido.count('INSERT INTO "review"'),
            'favoritos': contenido.count('INSERT INTO "favorite"'),
            'listas': contenido.count('INSERT INTO "reading_list"'),
            'notificaciones': contenido.count('INSERT INTO "notification"')
        }
        
        print("\n=== ESTADÍSTICAS DEL ARCHIVO DE DATOS ===")
        for tipo, cantidad in tipos_inserts.items():
            print(f"{tipo.capitalize()}: {cantidad}")
        print(f"Total de líneas: {len(contenido.splitlines())}")
        print(f"Tamaño del archivo: {len(contenido)} caracteres")

    def mostrar_informacion_perfiles(self):
        """Muestra información sobre los perfiles de usuario disponibles"""
        print("\n" + "="*60)
        print("👥 PERFILES DE USUARIO DISPONIBLES")
        print("="*60)
        
        perfiles_info = {
            'nuevo': {
                'emoji': '🆕',
                'probabilidad': '25%',
                'libros': '0-1',
                'reseñas': '0',
                'favoritos': '0',
                'listas': '0-1',
                'descripcion': 'Usuario nuevo con poca o ninguna actividad'
            },
            'casual': {
                'emoji': '📚',
                'probabilidad': '40%',
                'libros': '1-3',
                'reseñas': '0-2',
                'favoritos': '0-2',
                'listas': '1-2',
                'descripcion': 'Usuario casual con actividad moderada'
            },
            'activo': {
                'emoji': '⭐',
                'probabilidad': '25%',
                'libros': '3-6',
                'reseñas': '2-5',
                'favoritos': '2-4',
                'listas': '2-4',
                'descripcion': 'Usuario activo con buena participación'
            },
            'power_user': {
                'emoji': '🏆',
                'probabilidad': '10%',
                'libros': '5-10',
                'reseñas': '4-8',
                'favoritos': '3-6',
                'listas': '3-6',
                'descripcion': 'Usuario muy activo, gran lector'
            }
        }
        
        for perfil, info in perfiles_info.items():
            print(f"\n{info['emoji']} {perfil.replace('_', ' ').upper()}")
            print(f"   Probabilidad: {info['probabilidad']}")
            print(f"   Libros: {info['libros']}")
            print(f"   Reseñas: {info['reseñas']}")
            print(f"   Favoritos: {info['favoritos']}")
            print(f"   Listas: {info['listas']}")
            print(f"   Descripción: {info['descripcion']}")
        
        print("\n" + "="*60)
        print("💡 Los perfiles se asignan automáticamente de forma aleatoria")
        print("   para crear una base de datos más realista y diversa.")
        print("="*60)
    
    def mostrar_informacion_funcionalidad(self):
        """Muestra información sobre la nueva funcionalidad"""
        print("\n" + "="*70)
        print("🆕 NUEVA FUNCIONALIDAD: USUARIOS CON LIBROS EXISTENTES")
        print("="*70)
        print("\n📖 ¿Qué hace?")
        print("   Esta funcionalidad permite generar usuarios que interactúan")
        print("   SOLO con libros que ya existen en el archivo data.sql,")
        print("   sin crear nuevos autores, editoriales o libros.")
        print("\n🎯 ¿Cuándo usar?")
        print("   • Cuando quieras agregar más usuarios a tu base de datos")
        print("   • Cuando tengas suficientes libros y quieras más interacciones")
        print("   • Para simular una base de datos más real con múltiples usuarios")
        print("\n⚙️ ¿Cómo funciona?")
        print("   1. Lee el archivo data.sql existente")
        print("   2. Extrae todos los libros disponibles")
        print("   3. Crea usuarios que interactúan con esos libros")
        print("   4. Genera reseñas, favoritos, progreso de lectura, etc.")
        print("\n🔧 Opciones disponibles:")
        print("   • Opción 3: Generar 1 usuario con libros existentes")
        print("   • Opción 4: Generar múltiples usuarios con libros existentes")
        print("\n📊 Beneficios:")
        print("   • Base de datos más realista")
        print("   • Múltiples usuarios interactuando con el mismo contenido")
        print("   • Datos más diversos para pruebas")
        print("   • No duplica libros innecesariamente")
        print("="*70)

    def menu_interactivo(self):
        """Menú interactivo para el generador"""
        while True:
            print("\n" + "="*70)
            print("🎯 GENERADOR DE DATOS BOOKMATE")
            print("="*70)
            print("1. Generar 1 conjunto de datos aleatorios (con nuevos libros)")
            print("2. Generar múltiples conjuntos de datos (con nuevos libros)")
            print("3. 🆕 Generar usuario(s) con libros existentes")
            print("4. 🆕 Generar múltiples usuarios con libros existentes")
            print("5. Mostrar estadísticas del archivo")
            print("6. Ver información de perfiles de usuario")
            print("7. 🆕 Ver información sobre la nueva funcionalidad")
            print("8. Salir")
            print("="*70)
            
            opcion = input("Selecciona una opción (1-8): ").strip()
            
            if opcion == '1':
                self.ejecutar()
            elif opcion == '2':
                try:
                    cantidad = int(input("¿Cuántos conjuntos de datos quieres generar? "))
                    if cantidad > 0:
                        self.generar_multiples_registros(cantidad)
                    else:
                        print("❌ La cantidad debe ser mayor que 0")
                except ValueError:
                    print("❌ Por favor ingresa un número válido")
            elif opcion == '3':
                result = self.generar_usuario_con_libros_existentes()
                if isinstance(result, tuple) and len(result) == 3:
                    datos_sql, perfil, config = result
                    if datos_sql:
                        perfil_info = f"{perfil.replace('_', ' ').title()} - {config['descripcion']} (Solo libros existentes)"
                        self.guardar_datos(datos_sql, perfil_info)
                        print("✅ Usuario generado exitosamente")
                    else:
                        print("❌ No se pudo generar el usuario")
                else:
                    print("❌ Error en la generación del usuario")
            elif opcion == '4':
                try:
                    cantidad = int(input("¿Cuántos usuarios quieres generar? "))
                    if cantidad > 0:
                        self.generar_multiples_usuarios_existentes(cantidad)
                    else:
                        print("❌ La cantidad debe ser mayor que 0")
                except ValueError:
                    print("❌ Por favor ingresa un número válido")
            elif opcion == '5':
                self.mostrar_estadisticas()
            elif opcion == '6':
                self.mostrar_informacion_perfiles()
            elif opcion == '7':
                self.mostrar_informacion_funcionalidad()
            elif opcion == '8':
                print("¡Hasta luego! 👋")
                break
            else:
                print("❌ Opción no válida. Por favor selecciona una opción del 1 al 8.")

# Ejecutar el generador
if __name__ == "__main__":
    generador = BookmateDataGenerator()
    # Preguntar al usuario si quiere usar el menú interactivo
    usar_menu = input("¿Quieres usar el menú interactivo? (s/n): ").strip().lower()
    
    if usar_menu in ['s', 'si', 'sí', 'y', 'yes']:
        generador.menu_interactivo()
    else:
        generador.ejecutar()